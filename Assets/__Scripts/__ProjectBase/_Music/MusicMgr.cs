...
[ManagedSingleton]
public class MusicMgr : Singleton
{
    ...
    */
    ...
    AudioMixer _mainMixer = SingletonManager.Get<ResourceMgr>()?.Load<AudioMixer>("_Music/Mixers/MainMixer");
    public MusicMgr()
    {
    ...
    public void PlayBkMusic(string name)
    {
        if(bkMusic == null)
        {
            ...
            obj.name = "BKMusic";
            SingletonManager.Get<MonoManager>()?.DontDestoryOnLoad(obj);
            bkMusic = obj.AddComponent<AudioSource>();
        }

        SingletonManager.Get<ResourceMgr>()?.LoadAsyn<AudioClip>("_Music/BackGround/" + name, (AudioClip) =>
        {
    ...
    public void CrossFading(string name)
    {
        if (bkMusic == null)
        {
            PlayBkMusic(name);
            bkMusic.volume = 0;
            SingletonManager.Get<MonoManager>()?.StartCoroutine(StartFade(bkMusic,5f, bkValue,false));
        }
        else
        {
            ...
            newBk.volume = 0;
            SingletonManager.Get<MonoManager>()?.StartCoroutine(StartFade(oldBk, 2f, 0,true));
            SingletonManager.Get<MonoManager>()?.StartCoroutine(StartFade(newBk, 2f, bkValue,false));
    ...
    public void PlaySound(string name, bool isloop, UnityAction<AudioSource> callBack = null)
    {
        if (soundObj == null)
        {
            ...
            soundObj.name = "Sound";
            SingletonManager.Get<MonoManager>()?.DontDestoryOnLoad(soundObj);

        }

        SingletonManager.Get<ResourceMgr>()?.LoadAsyn<AudioClip>("_Music/SoundEffect/" + name, (clip) =>
        {
    ...
    public IEnumerator StartFade(AudioSource audioSource, float duration, float targetVolume, bool needDestory)
    {
        ...
        if(needDestory) SingletonManager.Get<MonoManager>()?.Destory(audioSource);
        yield break;
    }
}
